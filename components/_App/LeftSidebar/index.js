...
const Sidebar = ({ toogleActive }) => {
  ...
  const fetchModulePermissions = async () => {
    try {
      const response = await fetch(`${BASE_URL}/User/GetRolePermissionByRolePermissionTypeId?roleId=${role}&permissionTypeId=1`, {
        method: "GET",
        headers: {
      ...
      const transformed = data.result.map(item => {
        const navPermission = item.permissionTypes.find(p => p.name === "Navigation");
        return {
          CategoryId: item.id,
      ...
      const updatedItems = rawItems.map(menu => {
        ...
        if (updatedMenu.subNav) {
          updatedMenu.subNav = updatedMenu.subNav.map(sub => {
            const matched = transformed.find(t => t.CategoryId === sub.categoryId);
            return {
              ...sub,
              ...
              <Link href='/dashboard/main'>
                {ProjectNo === 1 ?
                  <>
                    ...
                    <img
                      src="/images/db-logo.png" alt="Logo"
                      className='black-logo'
                    />
                    <img
                      src="/images/db-logo.png" alt="Logo"
